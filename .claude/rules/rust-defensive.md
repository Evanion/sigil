...
`validate()` MUST exercise the exact constructors and validators that `apply()` invokes — not a lenient pre-parse. A `validate()` that uses `unwrap_or_default()`, a partial parse, or a relaxed check while `apply()` calls a strict validating constructor (`Type::new`) is a contract violation: `validate()` reports success, then `apply()` fails, leaving the caller's "validate-before-apply" guarantee worthless and the mutation half-applied. Factor the shared construction into one helper (e.g., `build_entry()`) that BOTH `validate()` and `apply()` call, so there is a single fallible path and no second, more-lenient one. Add a symmetry test: an input that `apply()` would reject MUST also be rejected by `validate()`. Precedent: PR #77 (RF-004) — `AddFontEntry::validate` parsed the family with `unwrap_or_default()` (accepting an empty family) while `apply()` routed through `FontEntry::new` (rejecting it); a no-Unicode-name font passed validate and failed apply. Fixed by sharing `build_entry()` → both route `FontEntry::new`, plus a symmetry test.
