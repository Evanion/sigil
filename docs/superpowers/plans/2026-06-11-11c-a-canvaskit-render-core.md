...
**DEPENDS ON the Font Pipeline spec (sequenced before 11c-A).** Fonts come from the pipeline's `FontCollection`/`FontMgr` (built from pipeline-provided font buffers via `FontMgr.FromData`) — do NOT invent an ad-hoc bundle here. Consume the pipeline's font-resolution API to obtain the typeface/font-family for a node's `TextStyle`.
- [ ] Map `TextStyle` → Skia `ParagraphStyle`/`TextStyle`, resolving the font family through the Font Pipeline's `FontCollection`. Build a `ParagraphBuilder`, add text, `layout(width)`, `canvas.drawParagraph(para, x, y)`. Handle `text_align`, `text_decoration` (underline/strikethrough via Skia decoration), and `text_shadow` (Skia text shadow on the TextStyle). `.delete()` paragraph + builder per frame. **Document in a code comment** that text layout is Skia-shaped and may differ sub-pixel from the old Canvas-2D `measureTextLines` (the spec's permitted parity exception). Golden: multi-line wrapped + left/center/right aligned + underlined text.
...
**Resolved dependency:** the font-source question (raised in self-review) is settled — Sigil has no font binaries today, so a dedicated **Font Pipeline spec is sequenced before 11c-A** and provides the `FontCollection`/`FontMgr` that Task 14 consumes. This plan does NOT bundle fonts ad-hoc. See `docs/superpowers/research/2026-06-11-font-pipeline-architecture.md` and the 11c-A spec's Prerequisite note.
