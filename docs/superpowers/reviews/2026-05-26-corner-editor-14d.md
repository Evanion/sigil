...
- **RF-027** — PathBuilder type lives in `canvas/` but is reached from `panels/`; consider promoting to `frontend/src/types/path-builder.ts`. *Source: Architect (RF-007).*
- **RF-028** — Verify Storybook stories' unique UUIDs. *Source: Architect (RF-008).*
- **RF-029** — Document Popover wrapper's "controlled mode + hidden trigger" pattern as supported/discouraged. *Source: Architect (RF-010).*
- **RF-030** — `currentSmoothing()` returns silent 0.5 fallback when non-superellipse; gated, but add `console.warn` on the unreachable path. *Source: FE (FE-9).*
- **RF-031** — `as unknown as Corners` double-cast in `writeCorners` avoidable via explicit 4-tuple construction. *Source: FE (FE-5).*
- **RF-032** — `headerLabel` naming → `popoverHeaderLabel` for greppability. *Source: BE (Q14d-09).*
- **RF-033** — Slider `onChangeEnd` inline handler does 2 statements (call + signal set); extract to `commitSmoothingGesture(v)`. *Source: Compliance.*
- **RF-034** — No first-time discoverability cue for the preview's interactivity. Mitigated by RF-003 fix. *Source: UX (UX-004).*
- **RF-035** — "Mixed" state not surfaced on the hotspot itself, only inside the popover. Suggestion for next iteration. *Source: UX (UX-005).*
- **RF-036** — Smoothing slider lacks tick marks / numeric badge / calibration labels. *Source: UX (UX-007).*
- **RF-037** — Smoothing slider gesture test jsdom-limited; asserts only finite, not actual value. *Source: BE (Q14d-10).*
- **RF-038** — `corner-section-pipeline.test.tsx` could exercise more node-kind paths. *Source: BE.*
...
- **A11Y-013** — `role="group"` on popover root could be redundant given the `<h3>` header. Drop in conjunction with RF-010 (heading-level fix).
