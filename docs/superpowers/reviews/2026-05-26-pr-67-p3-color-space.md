...
Path 1 chosen. Final status:

### Status (final)

- **17 findings resolved** (all Critical, High, Major, Minor — including the central canvas-wiring RF-001).
- **2 wont-fix** (suggestion-only): RF-018 (HSL tooltip Figma reference — kept; useful disambiguation for migrating users), RF-019 (4-decimal precision — kept; matches CSS spec; revisit if user feedback).
...
- **Batch A** — Canvas renderer wiring (RF-001 + RF-006 + RF-014). Moved `colorToCss` to `frontend/src/canvas/color-fill.ts`; routed 8 dispatch sites through it. Updated spec §11. 4 commits.
- **Batch B** — Picker fixes + a11y polish (RF-002 + RF-003 + RF-011 + RF-013 + RF-015 + RF-016 + RF-017). Init mode from `props.color.space`; sync emit on mode switch + no-op guard; HexInput badge test; canvas-context docstring; CSS flex-shrink; OOG suppression in P3; `aria-describedby` for keyboard discoverability. 2 commits.
- **Batch C** — Rust validation (RF-004 + RF-005 + RF-008 + RF-012). Manual Deserialize on Color routing through validating constructors; `MIN/MAX_COLOR_CHANNEL` enforced via `new_srgb`/`new_display_p3`; `is_out_of_srgb_gamut` wired to `Color::is_out_of_srgb_gamut`; symmetric `finite_or_zero`. 1 commit.
- **Batch D** — TS math NaN guards (RF-007). `Number.isFinite` at entry of `srgbEotf`, `srgbOetf`, `multiplyMatrixVec3`. 1 commit.
- **Batch E** — `ColorDisplayMode` exhaustiveness sentinel (RF-009). New `types.test-d.ts`. 1 commit.
- **Batch F** — Parity fixture ground truth (RF-010). New `scripts/generate-p3-parity.mjs` from W3C matrices; values byte-identical to bootstrap → confirms both production paths were correct. 1 commit.

Total remediation: 10 commits.
