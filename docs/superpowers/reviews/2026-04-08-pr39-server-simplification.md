...
- **Fix:** Deferred — requires MCP tools to construct TransactionPayload. ~16 call sites need refactoring.
- **Status:** `deferred` — Requires follow-up PR. MCP mutations still work (legacy fallback triggers fetchPages on clients).
...
- **Status:** `wont-fix` — Pre-existing issue, not introduced by this PR. Tracked separately.
