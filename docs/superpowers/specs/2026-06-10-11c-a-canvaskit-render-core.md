...
**Date:** 2026-06-10
**Status:** ⚠️ SUPERSEDED (2026-06-11) on the engine choice. After spiking both CanvasKit and hb-gpu on real hardware, the render core was changed to a **custom WebGL2 shape renderer + hb-gpu text** (issue #43; ~12× smaller bundle, working COLRv1 emoji, resolution-independent text). See `docs/superpowers/research/2026-06-11-hb-gpu-spike.md` and the new render-core epic (RC-1…). **Carries over** to the new design: the `SceneRenderer` seam, the Display-P3 framebuffer recipe (`drawingBufferColorSpace='display-p3'`), the offscreen golden parity harness, the per-frame memory discipline, the chrome-on-2D-overlay split, and the frontend-only / delete-Canvas-2D / WebGL2-required posture. **Does not carry over:** CanvasKit/Skia as the engine. This file is retained for its reusable design; do not implement it as the engine.
