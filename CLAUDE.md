...
- Spawns a single `sigil-server` child process on fixed port 4680.
...
- Define all validation artifacts in `validate.rs`: numeric limit constants (`MAX_*`, `LIMIT_*`, `MIN_*`), character denylists (e.g., `FONT_FAMILY_FORBIDDEN_CHARS`), character allowlists, and any other validation predicate or set used in more than one location. Do not inline these in type definition files or command modules — inline copies diverge silently. **Adoption is part of the definition:** when you introduce a shared constant specifically to dedupe an existing literal (a broadcast path string, a magic number, a regex), the SAME commit MUST replace EVERY existing inline occurrence with a reference to the new constant — `rg` for the literal and convert all hits — and add a test asserting at least one consumer reads the constant (e.g., `assert_eq!(op.path, BROADCAST_PATH_FONT_ENTRY)`). A constant defined but not adopted at the call sites is worse than no constant: it looks deduplicated in review but the literals still diverge silently. Precedent: PR #77 (RF-007) — `sigil_state::BROADCAST_PATH_FONT_ENTRY` was defined for single-source-of-truth but both the GraphQL and MCP transports kept hardcoding the literal; fixed by switching both to the const and asserting `op.path == const` in parity tests.
...
### Multiplied Per-Item Caps Require an Aggregate Budget Cap

When two or more validation limits multiply into a worst-case resource footprint — a per-item size cap times a per-collection count cap (`MAX_EMBEDDED_FONT_BYTES` × `MAX_FONTS_PER_DOCUMENT`), a per-connection buffer times a max-connections cap, a per-file size times a max-files cap — the product is the real resource ceiling, and it MUST be bounded by an explicit aggregate constant, not left implicit. Two independently-reasonable per-item caps (32 MiB × 256 = 8 GiB) silently authorize a denial-of-service / out-of-memory footprint that neither cap's author intended.

When you add or change any limit that participates in such a product, the same PR MUST:
1. Define a named aggregate constant (`MAX_TOTAL_*`) for the bounded resource (total bytes, total handles, total connections), sized to the actual budget — not the product of the per-item caps.
2. Enforce it with a **running-sum check** at every boundary that grows the collection (each transport's add/insert path, and the load/deserialize path), check-before-apply. The per-item cap does not enforce the aggregate — a check on each item never sees the running total.
3. Add a `test_<aggregate_constant>_enforced` test that drives the running sum past the aggregate cap and asserts a typed rejection — symmetric across every transport (GraphQL, MCP, load).
4. On the load/deserialize path, prefer warn-and-skip over hard-fail so a hand-edited or migrated workfile that exceeds the cap still opens in a degraded-but-usable state; the warning MUST name the aggregate constant and the observed total.

This is the resource analog of the "Sequential Shutdown Phases Must Fit Within the Container Stop Grace" rule in `rust-defensive.md` (which bounds an aggregate of *time*). Same failure shape: independently-sane per-item bounds that multiply past a budget no one declared. Precedent: PR #77 (RF-001) — embedded-font per-file (32 MiB) and per-document (256) caps multiplied to ~8 GiB resident with no aggregate bound; fixed with `MAX_TOTAL_EMBEDDED_FONT_BYTES` (256 MiB) + running-sum enforcement at addFont (GraphQL+MCP, check-before-apply) and load_font_assets (warn+skip), with `_enforced` tests symmetric across transports.
